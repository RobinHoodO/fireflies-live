...
import { __test, connectLive, fetchFeed, fetchKeys } from "./backend.ts";
...
test("fetchKeys returns capability sentinels, never server credential fields", async () => {
  ...
  try {
    assert.deepEqual(await fetchKeys(), { ffKey: "server", orKey: "server", bridgeToken: "server", providerPolicyError: "" });
