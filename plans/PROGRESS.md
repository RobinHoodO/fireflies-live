...
| 015 | _(see git log)_ | `.playwright-mcp/`, `after-spacing.png`, `*.tsbuildinfo` gitignored; the two tracked artifacts untracked (working files kept). `callAI` failures now name the model slug + HTTP status + OpenRouter error detail instead of a generic "Couldn't generate a response." Model-roster auto-fetch from OpenRouter `/models` noted as deferred. |
| 014 | _(see git log)_ | Key gate fails closed: absent `sec-fetch-site` now 403s (curl-verified both directions; the app's own fetch still passes). `rootedPath` is realpath-based and async, all 4 call sites awaited. **Plan's STOP condition handled:** `clients/Katapult Future Fest` is a real first-level symlink, so instead of the naive whole-root realpath check the rule trusts first-level operator-curated symlinks and requires everything beneath to resolve inside that folder's real location. Discovered pre-existing: first-level symlinked client folders never resolved in `/context` anyway (`dirent.isDirectory()` is false for symlinks), so nothing regressed. Symlink-escape test added (planted `leak.md` → outside file never enters the bundle). |
| 013 | _(see git log)_ | FEATURES.md: persistence + shortcuts flipped to ✅ (Phase 2, 2026-07-12), model-picker note corrected (~22 grouped, Claude Sonnet 5 default), filter taxonomy → ask/do/note/command, react-markdown note updated post-v1-retirement, header now names ROADMAP.md as source of truth. INTEGRATION_PLAN.md: standalone-HTML path corrected from ~/Desktop to repo root. Docs-only diff. |
| 012 | _(see git log)_ | Audit log is metadata-only by default: `RUN`/`PI` log byte counts, `BLOCKED`/`PI-BLOCKED` log the matched pattern, `CTX` keeps only the source-kind:count summary; `BRIDGE_AUDIT_BODIES=1` restores body logging. `BRIDGE_AUDIT_FILE` env override added; test asserts a benign `/run` command's raw text never lands in the log by default. Log rotation noted as a deferred follow-up. |
| 011 | _(see git log)_ | `fl-config` write debounced 400ms with a `pagehide` flush (no lost final keystrokes on tab close); live round-trip verified — typed goal survives reload. Idle tick coarsened 5s→30s: chose the plan's not-fully-memoized branch since the rel-time labels live in the (unmemoized) sidebar. |
